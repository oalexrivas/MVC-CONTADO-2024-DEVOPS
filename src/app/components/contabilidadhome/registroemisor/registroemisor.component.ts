...
@Component({
  ...
  templateUrl: './registroemisor.component.html',
  styleUrls: ['./registroemisor.component.css']
})
export class RegistroEmisorComponent implements OnInit {
  ...
  obtenerUsuarios(): void {
    ...
    this.http.post<any>('http://localhost:4000/', { query }, { headers }).subscribe({
      next: result => {
        const usuarioData = result.data.obtenerPersona;
        console.log(usuarioData);
        const lista = usuarioData.map((item: any) => ({
          correo: item.nombreComercial,
          nombre: item.nit,
          apellido: item.nrc, 
